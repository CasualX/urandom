...
/// # Panics
///
/// `IoRead` uses [`io::Read::read_exact`], which retries on interrupts.
...
/// let mut rand = urandom::rng::IoRead::new(&data[..]);
///
...
impl<R> IoRead<R> {
	...
	#[inline]
	pub fn new(reader: R) -> Random<IoRead<R>> {
		Random::from(IoRead { reader })
	}
}

impl<R: io::Read> Rng for IoRead<R> {
	#[inline]
