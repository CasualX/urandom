...
pub trait NormalImpl<Float>: Sized {
	fn new(_1: Float, _2: Float) -> Result<Self, NormalError>;
	fn from_mean_cv(_1: Float, _2: Float) -> Result<Self, NormalError>;
...
///
/// ```
/// use urandom::distr::{Normal, Distribution};
///
...
impl<Float: Copy> Normal<Float> where Self: NormalImpl<Float> {
	...
	#[inline]
	pub fn new(mean: Float, std_dev: Float) -> Result<Normal<Float>, NormalError> {
		NormalImpl::new(mean, std_dev)
	}
	...
	#[inline]
	pub fn from_mean_cv(mean: Float, cv: Float) -> Result<Normal<Float>, NormalError> {
		NormalImpl::from_mean_cv(mean, cv)
	}
...
macro_rules! impl_normal {
	($ty:ty) => {
		impl NormalImpl<$ty> for Normal<$ty> {
			#[inline]
			fn new(mean: $ty, std_dev: $ty) -> Result<Normal<$ty>, NormalError> {
			...
			#[inline]
			fn from_mean_cv(mean: $ty, cv: $ty) -> Result<Normal<$ty>, NormalError> {
...
///
/// ```
/// use urandom::distr::{LogNormal, Distribution};
///
/// // mean 2, standard deviation 3
...
impl<Float: Copy> LogNormal<Float> where Self: NormalImpl<Float> {
	...
	#[inline]
	pub fn new(mu: Float, sigma: Float) -> Result<LogNormal<Float>, NormalError> {
		NormalImpl::new(mu, sigma)
	}
	...
	#[inline]
	pub fn from_mean_cv(mean: Float, cv: Float) -> Result<LogNormal<Float>, NormalError> {
		NormalImpl::from_mean_cv(mean, cv)
	}
...
macro_rules! impl_log_normal {
	($ty:ty) => {
		impl NormalImpl<$ty> for LogNormal<$ty> {
			#[inline]
			fn new(mu: $ty, sigma: $ty) -> Result<LogNormal<$ty>, NormalError> {
				let norm = Normal::new(mu, sigma)?;
				Ok(LogNormal { norm })
			}

			#[inline]
			fn from_mean_cv(mean: $ty, cv: $ty) -> Result<LogNormal<$ty>, NormalError> {
				if cv == 0.0 {
					let mu = mean.ln();
					let norm = Normal::new(mu, 0.0).unwrap();
