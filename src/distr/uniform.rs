...
///
/// ```
/// use urandom::distr::{Distribution, Uniform};
///
...
/// use urandom::distr::{SampleUniform, UniformFloat, UniformSampler};
///
...
impl<T: SampleUniform> From<ops::Range<T>> for Uniform<T> {
	...
	fn from(range: ops::Range<T>) -> Uniform<T> {
		Uniform { sampler: T::Sampler::new(range.start, range.end) }
	}
}
impl<T: SampleUniform> From<ops::RangeInclusive<T>> for Uniform<T> {
	...
	fn from(range: ops::RangeInclusive<T>) -> Uniform<T> {
		...
		Uniform { sampler: T::Sampler::new_inclusive(start, end) }
	}
}
